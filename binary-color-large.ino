...
CapacitiveSensor s1 = CapacitiveSensor(3, 4);
CapacitiveSensor s2 = CapacitiveSensor(3, 5);
CapacitiveSensor s3 = CapacitiveSensor(3, 6);
CapacitiveSensor s4 = CapacitiveSensor(3, 7);
CapacitiveSensor s5 = CapacitiveSensor(3, 8);
CapacitiveSensor s6 = CapacitiveSensor(3, 9);
CapacitiveSensor s7 = CapacitiveSensor(3, 10);
CapacitiveSensor s8 = CapacitiveSensor(3, 11);
CapacitiveSensor s9 = CapacitiveSensor(3, 12);
CapacitiveSensor *sensors[] = {&s1, &s2, &s3, &s4, &s5, &s6, &s7, &s8, &s9};

byte bytes[] = {0, 0, 0};
int currentByteIndex = 0;
int currentRingSpinPixel = 0;
...
Adafruit_NeoPixel rings = Adafruit_NeoPixel(RING_SIZE * 3, RING_PIN, NEO_GRB + NEO_KHZ800);
...
void loop() {
  checkSensors();
  spinRing(currentByteIndex, currentByteColor32());
}

void checkSensors() {
  if (checkSensorPressed(0)) {
    fillRing(currentByteIndex, currentByteColor32());
    for (int i = 0; i < 8; i++) {
      resetJewel(i);
    }
    currentByteIndex++;
...
void fillRing(int i, uint32_t color) {
  resetRing(i);
  for (int j = RING_SIZE * i; j < RING_SIZE * (i + 1); j++) {
    rings.setPixelColor(j, color);
    rings.show();
    delay(3);
  }
}
...
void resetRing(int i) {
  for (int j = RING_SIZE * i; j < RING_SIZE * (i + 1); j++) {
    rings.setPixelColor(j, 0);
  }
  rings.show();
}

void spinRing(int i, uint32_t color) {
  resetRing(i);
  rings.setPixelColor(RING_SIZE * i + currentRingSpinPixel, color);
  currentRingSpinPixel = (currentRingSpinPixel + 1) % RING_SIZE;
  rings.show();
}
